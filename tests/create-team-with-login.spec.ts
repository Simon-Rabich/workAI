...
// });
import { test, expect } from '@playwright/test';
import { getOtpFromGmail } from '../utils/getOtp';

test('Login and create a team with OTP', async ({ page }) => {
  await page.goto('https://start.ai.work/');
  ...
  // 2. Wait and retrieve OTP from email
  const otpCode = await getOtpFromGmail();

  // 3. Enter each digit (assuming OTP has 6 input boxes)
  const digits = otpCode.split('');
  for (let i = 0; i < digits.length; i++) {
    await page.locator(`input[autocomplete="one-time-code"] >> nth=${i}`).fill(digits[i]);
  }
  ...
  // 5. Continue with creating a team
  await page.locator('div.TooltipOnOverflow__trigger span:has-text("Home")').click();
  await page.locator('span:has-text("Create team")').click();
  await expect(page).toHaveURL(/.*work.*/);
});
